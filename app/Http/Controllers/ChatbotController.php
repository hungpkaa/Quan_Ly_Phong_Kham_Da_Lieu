...
9. Nếu người hỏi hỏi về thời gian làm việc thì nói kiểm tra ở trang Doctors của website phòng khám da liễu PHENIKAAMEC.

10. Nhớ không được trả lời dài dòng, phải đúng trọng tâm.";

        try {

            // Gửi request đến Gemini API mới
        $response = Http::withHeaders([
    'Authorization' => 'Bearer ' . $this->apiKey,
    'HTTP-Referer' => 'http://127.0.0.1:8000',
    'X-Title' => 'PhenikaaMec AI',
...
if (isset($responseData['error'])) {
    ...
    Log::error('OpenRouter Error: ' . json_encode($responseData['error']));

    return response()->json([
        'message' => 'Chatbot hiện đang lỗi hoặc API không hoạt động.'
    ], 500);
}
...
if (
    ...
    Log::error('Invalid AI Response: ' . json_encode($responseData));
...
return response()->json([
    ...
    return response()->json([
        'message' => 'Không thể kết nối tới AI.',
        'error' => $e->getMessage()
    ], 500);
}
}
